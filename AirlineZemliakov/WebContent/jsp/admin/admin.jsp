							...
							<div class="tab-content" id="Flight">
								<form id="form_4" action="controller" method="post">
									...
									<div>
										...
										<c:if test="${ flightNotAdded != null }"><fmt:message key="${ flightNotAdded }" /></c:if>
										<c:if test="${ flightAdded != null }"><fmt:message key="${ flightAdded }" /></c:if>
										</span>
									</div>
								</form>
							</div>
								...
								<form id="form_4" action="controller" method="post">
									...
									<div>
										...
										<span class="left"><fmt:message key="flight.flight" />
												...
												<select class="input" name="delFlightId">												
												<c:forEach var="delFlight" items="${delFlights}">
													<option value="${ delFlight.id }">${ delFlight }</option>
												</c:forEach>
											</select>
										...
										<c:if test="${ flightDeleted != null }"><fmt:message key="${ flightDeleted }" /></c:if>
										<c:if test="${ flightNotDeleted != null }"><fmt:message key="${ flightNotDeleted }" /></c:if>
										</span>
									</div>
								</form>
							</div>
				...
				<article class="col2">
					<div class="box1" style="margin-top: 280px;">
						...
						<div>
							...
							<span>
							<c:if test="${ flightCompleted != null }"><fmt:message key="${ flightCompleted }" /></c:if>
							<c:if test="${ flightNotCompleted != null }"><fmt:message key="${ flightNotCompleted }" /></c:if>
							</span>
						</div>
					</div>
				</article>
