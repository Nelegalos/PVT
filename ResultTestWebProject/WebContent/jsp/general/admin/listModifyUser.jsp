...
<body>
	...
	<form action="Test" method="post">
		<input name="page" type="hidden" value="listModifyUser" />
		...
		<table>
			<c:forEach items="${tutors}" var="tutor">
				<tr>
					<td class = "bord1px">
						<p>
							<input type="radio" name="userId" value="${tutor.id}" />
								<fmt:message key="tutor" />#${tutor.id}: ${tutor.firstName} ${tutor.lastName}<br>
						</p>
					</td>
				</tr>
			</c:forEach>
			<c:forEach items="${students}" var="student">
				<tr>
					<td class = "bord1px">
						<p>
							<input type="radio" name="userId" value="${student.id}" />
